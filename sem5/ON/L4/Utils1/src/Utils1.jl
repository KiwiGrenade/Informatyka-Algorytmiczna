...
function rysujNnfx(f, a :: Float64, b :: Float64, n :: Int)
    x = zeros(n+1)        # wektor zawierający węzły
    y = zeros(n+1)        # wartości funkcji w kolejnych węzłach
    fx = zeros(n+1)       # ilorazy różnicowe dla kolejnych węzłów

    dens = 20                       # mnożnik dla dokładiejszego rysowania wykresów

    img_y = zeros(dens*(n + 1))       # wektor wartości funkcji na rysunku
    img_x = zeros(dens*(n + 1))       # we    max_k = n + 1ktor węzłów na rysunku
    img_new = zeros(dens*(n + 1))     # wartości wielomianu interpolacyjnego w węzłach
